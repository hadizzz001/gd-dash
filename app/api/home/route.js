...
export async function GET(req) {
  try {
    const categories = await prisma.home.findMany();
...
export async function POST(req) {
  try {
    const { description,sdescription, last, img } = await req.json(); 

    console.log("body are: " ,sdescription );
    
    
    const category = await prisma.home.create({ data: { description,sdescription, last, img} });
...
export async function PATCH(req) {
  ...
  try {
    ...
    const updatedCategory = await prisma.home.update({
      where: { id },
      data: { description,sdescription, last, img },
    });
...
export async function DELETE(req) {
  try {
    ...
    const deletedCategory = await prisma.home.delete({ where: { id } });
